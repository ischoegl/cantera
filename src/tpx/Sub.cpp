...
{
    double Tsave = T;
    // The triple-point pressure is not stored as a numerical parameter, so it
    // is evaluated as the saturation pressure at the minimum temperature.
    T = Tmin();
    double Ptriple = Ps();
    T = Tsave;
    if (p <= 0. || p > Pcrit() || p < Ptriple) {
...
void Substance::set_TPp(double Temp, double Pressure)
{
    ...
    while (P_here = Pp(),
        ...
        if (have_prev && kbr == 0 && res*res_prev < 0.0) {
            // The residual changed sign between the previous and current
            // specific volume, so the root lies between them. On the steep
            // liquid branch at low temperature the iteration can step across
            // the (very thin) target band without the logic below ever
            // recording a bracket, leaving the solver unable to converge
            // (GitHub issue #605). Capture the bracket explicitly so the
            // in-bracket refinement below can proceed. Pressure decreases with
            // specific volume here, so the smaller volume is the lower bound.
            if (v_here < v_prev) {
                Vmin = v_here;
                Pmin = P_here;
                Vmax = v_prev;
        ...
        if (fabs(dv) <= 4.0*std::numeric_limits<double>::epsilon()*fabs(v_here)) {
            // The volume step has dropped below the resolution of the specific
            // volume in double precision, so no nearer representable volume
            // brings the pressure closer to the target. This is convergence to
            // floating-point precision rather than a failure (GitHub issue
            // #605): it occurs on the nearly incompressible liquid branch at
            // low temperature, where the saturation pressure is so small that
            // the requested relative pressure tolerance is finer than one
            // floating-point ULP of pressure, and when the initial density
            // guess already sits at the root.
            break;
        }
